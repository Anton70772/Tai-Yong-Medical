...
var displayIntro = (port) => {
  ...
  console.log(
    `
 ${bold(magenta(`\u26A1 Million Lint v${"1.0.0-rc.12"}`))}
...
// compiler/src/core/bridge.ts
var ENDPOINT = "https://million-lint-git-feat-encodingbatch-millionjs.vercel.app/api/v1/ingest";
...
async function* getPossibleConnections(port = DEFAULT_PORT) {
  for (const host of ["localhost", "0.0.0.0"]) {
    ...
    for (const window of responses.flat()) {
      if (!window)
        continue;
      yield {
        host,
...
ingestServer.use((0, import_cors.cors)()).use(decompress()).post("/ingest", async (c) => {
  ...
  return c.json({ ok: Boolean(sessions.length) });
}).get("/data", (c) => {
...
var _startIngestServer = async (host, port) => {
  ...
  return new Promise((resolve) => {
    const server2 = (0, import_node_server.serve)({
...
var buildCacheHandler = (cwd2, file) => {
  return {
    write: (data) => {
      const baseDir = import_node_path2.default.join(cwd2, ".million");
      try {
...
var unplugin = (0, import_unplugin.createUnplugin)((options, meta) => {
  ...
  if (!currentOptions.test && process.env.NODE_ENV !== "production") {
    void startIngestServer(currentOptions.ingest);
    void reset2();
  }
  const stats = {
    components: 0,
  ...
  return {
    ...
    async transform(code, id) {
      ...
      const relativePath = import_node_path3.default.relative(cwd, id);
...
var unwrapBody = (path5) => {
  if (path5.isFunctionDeclaration() || path5.isFunctionExpression() || path5.isArrowFunctionExpression() || path5.isProgram()) {
    ...
    if (t5.isBlockStatement(path5.node.body)) {
...
var wrapHMR = (ctx, path5, expression) => {
  ...
  const init2 = getImportIdentifier(ctx, path5, {
    kind: "named",
    name: "init",
    source: RUNTIME_SOURCE
  });
...
var JSX_RE = /<(?<temp1>"[^"]*"|'[^']*'|[^'">])*>/;
var hasJSX2 = (path5) => {
  if (JSX_RE.test(path5.getSource()))
    return true;
  let hasJSX3 = false;
  path5.traverse({
    JSXElement(path6) {
...
var getExternals = (path5, componentName) => {
  const externals = {};
  const bindings = path5.scope.getAllBindings();
  for (const bindingName in bindings) {
    ...
    if (bindingValue && bindingName === componentName)
      continue;
    if (bindingValue.scope.uid === path5.scope.uid) {
      continue;
    }
    if (cache.has(key)) {
...
var registerMetadata = (ctx, path5, key, index, componentName, checkScope = true) => {
  var _a;
  ...
  if (!t7.isArrowFunctionExpression(path5.node) && t7.isIdentifier(path5.node.id) && !(scope == null ? void 0 : scope.path.scope.hasBinding(path5.node.id.name))) {
    return;
  }
  ctx.queue.push(() => {
    ...
    const body = unwrapBody(checkScope ? scope.path : path5);
    if (!body)
      return;
    if (process.env.NODE_ENV === "production") {
      body.push(t7.expressionStatement(register));
    } else {
      body.push(...wrapHMR(ctx, checkScope ? scope.path : path5, register));
    }
  });
  const filename = ctx.options.filename;
  const loc = unwrapLoc(path5.node.loc);
  const nameLoc = unwrapLoc((_a = getDescriptiveNamePath(path5)) == null ? void 0 : _a.node.loc);
  ...
  file.components[componentName] = {
    externals: getExternals(path5, componentName),
...
var getDefinitionFromExpression = (ctx, path5, registration) => {
  const id = unwrapNode(path5.node, t8.isIdentifier);
  if (id) {
  ...
  const memberExpr = unwrapNode(path5.node, t8.isMemberExpression);
...
var transformCustomCaptureCall = (ctx, key, path5, index, mountInfo) => {
  const callee = path5.get("callee");
  if (!callee.isExpression()) {
    return false;
  }
  ...
  args[2] = t9.numericLiteral(encode(unwrapLoc(path5.node.loc)));
...
var getCaptureProps = (ctx, path5, capture, key, mountInfo, seen) => {
  ...
  path5.node.params = [propsID, t9.restElement(restID)];
  return [
    t9.variableDeclaration("let", [
...
var transformComponent = (ctx, path5, type) => {
  ...
  const capture = getImportIdentifier(ctx, path5, ANYA_CAPTURE);
  ...
  const loc = encode(unwrapLoc(path5.node.loc));
  const mountInfo = generateUniqueName(path5, "_$");
  ...
  if (type === "component") {
    replacements.push(
      ...getCaptureProps(ctx, path5, capture, key, mountInfo, seen)
    );
  }
  if (isPathValid(path5, t9.isArrowFunctionExpression)) {
  ...
  } else if (isPathValid(path5, t9.isFunctionExpression) || isPathValid(path5, t9.isFunctionDeclaration)) {
  ...
  path5.traverse({
    CallExpression(childPath) {
      ...
      if (!(functionParent && functionParent === path5)) {
        return;
      }
    ...
    JSXElement(childPath) {
      ...
      const loc2 = encode(unwrapLoc(childPath.node.loc));
      const containerPath = path5.find((p) => p.isJSXExpressionContainer());
  ...
  registerMetadata(ctx, path5, key, index, componentName);
};
...
var extractImportIdentifiers = (ctx, path5) => {
  ...
  if (mod.includes("@million/lint/runtime") && mod !== RUNTIME_SOURCE) {
    path5.node.source.value = RUNTIME_SOURCE;
  }
...
var transformInit = (ctx, path5) => {
  ...
  if (definition && definition.kind === "named" && definition.name === "init") {
    path5.node.arguments[0] = t10.objectExpression([
      t10.objectProperty(
        t10.identifier("url"),
...
var transformVariableDeclarator = (ctx, path5) => {
  if (path5.node.init && t10.isIdentifier(path5.node.id)) {
    const componentNode = path5.get("init");
    const type = getTypeFromComponentName(ctx, componentNode, path5.node.id);
    if (!type)
      return;
    transformFunction(ctx, componentNode, type);
    const id = path5.node.id;
    path5.get("init").traverse({
      Identifier(childPath) {
        const key = ctx.componentKeys[childPath.node.name];
        if (key) {
          registerMetadata(ctx, path5, key, -1, id.name, false);
          childPath.stop();
        }
      }
    });
      ...
      Program(programPath) {
        ...
        programPath.traverse({
          ImportDeclaration(path5) {
            extractImportIdentifiers(ctx, path5);
          }
        });
        ...
        programPath.traverse({
          CallExpression(path5) {
            transformInit(ctx, path5);
            transformHOC(ctx, path5);
          },
          FunctionDeclaration(path5) {
            transformFunction(ctx, path5, getComponentType(ctx, path5));
          },
          FunctionExpression(path5) {
            transformFunction(ctx, path5, getComponentType(ctx, path5));
          },
          VariableDeclarator(path5) {
            transformVariableDeclarator(ctx, path5);
          }
...
async function MillionLintLoader(code) {
  ...
  try {
    ...
    const withBabelOptions = Object.assign(
      {
        source: code,
        filename: import_node_path4.default.relative(process.cwd(), id),
        framework: "webpack",
        absoluteFilename: id,
        stats: {
          components: 0,
    ...
    if (!result) {
      throw new TypeError(
        "Failed to transform code. Please check the console for more details."
      );
    }
    callback(null, result.code ?? void 0, result.map ?? void 0);
  } catch (e) {
    callback(e);
  }
}
